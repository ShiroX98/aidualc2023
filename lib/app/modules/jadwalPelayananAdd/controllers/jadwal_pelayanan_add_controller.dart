...
class JadwalPelayananAddController extends GetxController {
  String namaIbadah = 'Doa Pagi';
  RxBool isLoading = false.obs;
  RxBool isHidden = true.obs;
  var tanggalIbadah = DateTime.now().obs;
  var waktuIbadah = TimeOfDay.now().obs;
  var wl = "".obs;
  var uidwl = "".obs;
  var lcd = "".obs;
  var uidlcd = "".obs;
  ...
  var jadwalPelayananUid = "";

  FirebaseAuth auth = FirebaseAuth.instance;

  FirebaseFirestore firestore = FirebaseFirestore.instance;

  pilihNamaIbadah(value) async {
    namaIbadah = value;
    print(namaIbadah);
  }
  ...
  pilihTanggal() async {
    DateTime? pickedDate = await showDatePicker(
        context: Get.context!,
        initialDate: tanggalIbadah.value,
        firstDate: DateTime(1945),
        lastDate: DateTime(2100));

    if (pickedDate != null && pickedDate != tanggalIbadah.value) {
      tanggalIbadah.value = pickedDate;
    }
  }
  ...
  pilihWaktu() async {
    TimeOfDay? pickedTime = await showTimePicker(
      context: Get.context!,
      initialTime: waktuIbadah.value,
      builder: (context, child) {
        return Theme(data: ThemeData.dark(), child: child!);
      },
      initialEntryMode: TimePickerEntryMode.input,
      helpText: 'Pilih Waktu Ibadah',
      cancelText: 'Tutup',
      confirmText: 'OK',
      errorInvalidText: 'Pilih waktu yang benar.',
      hourLabelText: 'Pilih Jam',
      minuteLabelText: 'Pilih Menit',
    );
    if (pickedTime != null && pickedTime != waktuIbadah.value) {
      waktuIbadah.value = pickedTime;
    }
  }
  ...
  void batalAddPelayanan() async {
    ...
    Get.offAllNamed(Routes.JADWAL_PELAYANAN, arguments: levelValue);
  }
  ...
  void simpanPelayanan() async {
    if (wl.value.isNotEmpty &&
      ...
      try {
        isLoading.value = true;
        var hour = waktuIbadah.value.hour.toString().padLeft(2, '0');
        var minute = waktuIbadah.value.minute.toString().padLeft(2, '0');
        // print("${hour}:${minute}");
        await firestore.collection("jadwal_pelayanans").add({
          "nama_ibadah": namaIbadah,
          "tanggal_ibadah": tanggalIbadah.toString(),
          "waktu_ibadah": "$hour:$minute",
          "createdAt": DateTime.now().toIso8601String(),
        });

        var collectionTest = firestore.collection("jadwal_pelayanans");
        var snapshotsTest = await collectionTest.get();
        for (var doc in snapshotsTest.docs) {
          var collectionTestSub =
              doc.reference.collection("petugas_pelayanans");
          var snapshotsTestSub = await collectionTestSub.get();
          if (snapshotsTestSub.docs.isEmpty) {
            jadwalPelayananUid = doc.id.toString();
          }
        }
        DocumentSnapshot<Map<String, dynamic>> doc = await firestore
            .collection("jadwal_pelayanans")
            .doc(jadwalPelayananUid.toString())
            .get();
        Map<String, dynamic>? jadwalPelayanan = doc.data();
        //! Store WL
        await firestore
            .collection("jadwal_pelayanans")
            .doc(jadwalPelayananUid)
        ...
        await firestore
          ...
          "nama_ibadah": jadwalPelayanan?['nama_ibadah'],
          "tanggal_ibadah": jadwalPelayanan?['tanggal_ibadah'],
          "waktu_ibadah": "$hour:$minute",
          "petugas": "WL",
        });
        //! End Store WL
        ...
        isLoading.value = false;
        Get.offNamed(Routes.JADWAL_PELAYANAN, arguments: levelValue);
      } catch (e) {
        isLoading.value = false;
        print(e);
        Get.snackbar(
